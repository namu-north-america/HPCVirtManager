...
export const updateYamlString = ({
  ...
  const isCustom = data.virtualMachineType === "custom";

  if (data.virtualMachineType !== "custom") {
    virtualMachine.spec.template.spec["instancetype"] = {
      kind: "VirtualMachineClusterInstancetype",
      name: data.virtualMachineType,
    };
    delete virtualMachine.spec.template.spec.domain.cpu;
  } else {
    virtualMachine.spec.template.spec.domain.cpu = {};
    delete virtualMachine.spec.template.spec["instancetype"];
  }
    ...
    disks.forEach((disk, i) => {
      if (disk.createType === "new" || disk.createType === "image") {
        ...
        if (i === 0 && disks.length === 1 && useVmTemplate) {
          ...
          const storage = disk.size ? `${disk.size}${disk?.memoryType}` : item.spec?.pvc?.resources?.requests?.storage;
